...
int new_improve_solution(int **nd, int *nd_len, int n, int *parent, int *vtx_depth, int max_depth, bool *made_a_change)
{
    *made_a_change = false;
    int *path_vv = malloc(n * sizeof *path_vv);
    int *child = malloc(n * sizeof *child);  // a child of each vertex that has a child
    int *vtx_num_children = calloc(n, sizeof *vtx_num_children);

    for (int i=0; i<n; i++)
        if (parent[i] != -1)
            ++vtx_num_children[parent[i]];
    ...
    bool *vertex_has_been_tried = calloc(n, sizeof *vertex_has_been_tried);
    ...
    for (int i=0; i<n; i++) {
        ...
        int z = i;
        while (z != -1) {
            if (!vertex_has_been_tried[z] && (parent[z] == -1 || vtx_num_children[parent[z]] > 1)) {
                // z is not an only child
                path_vv[0] = z;
                int path_vv_len = 1;
                int v = z;
                while (vtx_num_children[v] == 1) {
                    v = child[v];
                    path_vv[path_vv_len++] = v;
                }
                // path_vv is now z followed by its path of only-child descendents
                if (vtx_num_children[v] == 0) {
                    // path_vv are a "lowest chain" in the terminology used by Fredrik Manne.
                    // If we can find two non-adjacent vertices in the chain, we can
                    // put them at the bottom as siblings.
                    for (int j=0; j<path_vv_len-1; j++) {
                        int t = path_vv[j];
                        for (int k=j+1; k<path_vv_len; k++) {
                            int u = path_vv[k];
                            if (are_adj(t, u, nd, nd_len)) {
                                continue;
                            }
                            // t and u are not adjacent
                            if (t != v && u != v) {
                                parent[child[t]] = parent[t];
                                parent[child[u]] = parent[u];
                                parent[t] = v;
                                parent[u] = v;
                            } else if (u == v) {
                                parent[child[t]] = parent[t];
                            ...
                            // recalculate depths
                            max_depth = calculate_depths(vtx_depth, parent, n);

                            *made_a_change = true;
                            goto cleanup_new_improve_solution;
                        }
                    }
                } else {
                    // The chain does not end at a leaf node.  Loof for a vertex w in the chain
                    // that only has neighbours in one of the subtrees below the chain.
                    // TODO: improve this from "one of the subtrees" to "not all of the subtrees"
                    int v_depth = vtx_depth[v];
                    for (int j=0; j<path_vv_len-1; j++) {
                        int w = path_vv[j];
                        assert(w != v);
                        int a = -1;   // the root of a "used" subtree
                        for (int k=0; k<nd_len[w]; k++) {
                            int u = nd[w][k];
                            int u_ancestor = get_vtx_ancestor_at_depth(u, v_depth+1, parent, vtx_depth);
                            if (u_ancestor != -1) {
                                assert (parent[u_ancestor] == v);
                                if (a == -1) {
                                    a = u_ancestor;
                                } else if (a != u_ancestor) {
                                    a = -2;
                                    break;
                                }
                            }
        ...
        //                    printf("%d %d\n", v, w);
            ...
            vertex_has_been_tried[z] = true;
            z = parent[z];
        }
    }

cleanup_new_improve_solution:

    free(vtx_num_children);
    free(child);
    free(path_vv);
    free(vertex_has_been_tried);
            ...
            for (int k=0; k<top_k; k++) {
                ...
                if (i > 100) {
                    for (int j=0; j<1000 + i; j++) {
                        ...
                        new_d = new_improve_solution(nd, nd_len, n, parent, vtx_depth, new_d, &made_a_change);
                        // TODO: maybe improve small changes found within improve_solution() too
                        if (!made_a_change)
                            break;
                    }
                }
