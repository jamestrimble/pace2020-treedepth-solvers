...
//    ++tmp_counter;
    ...
    for (int i=0; i<node->successor_len; i++) {
        ...
        if (new_remaining_num_additions_permitted >= 0 && popcount_of_set_difference(succ->subtree_intersection, set) <= num_additions_permitted &&
...
{
    *arr_out_len = 0;
    trie_get_all_almost_subsets_helper(trie, &trie->root, set, aux_set,
            num_additions_permitted, num_additions_permitted, arr_out, arr_out_len);
}
